...
def get_vault_tx_steps(
    ...
    api_response = api.repay_zar_transaction(request)
    return api_response.number_of_steps, api_response.step_number, api_response.steps
            ...
            for s in range((num_of_steps - step_number) + 1):
                ...
                for number, step in enumerate(steps):
                    ...
                    if (step_number - 1) == number:
                        ...
                        if receipt is None:
                            print(f"Transaction 0x{tx_hash} was not mined within the timeout period.")
                            continue  # Skip to the next step if this transaction wasn't mined

                        print(f"Transaction 0x{tx_hash} was mined in block {receipt['blockNumber']}")
        else:
